...
\par
A position estimate for every feature is calculated to achieve system requirement \hyperref[req3.1]{3.1} and \hyperref[req3.2]{3.2}. The centroid is calculated in one of two ways, the weighted and non-weighted version.
\par 
The weighted centroid of a feature estimates the true center of it. The process takes into account pixel brightness values and distance from the average coordinate. An average coordinate is the value between the minimum and maximum coordinate, stored in the feature table. The process creates an integer value for every row and column of the feature using these calculations. Each pixel in the feature contributes with its brightness, while the column, or row number contributes with the weight. The calculation can be described with the following expression $C_{i} = C_{i} + \texttt{0xffff} - F_{jk}|i-c|$, where $C_{i}$ is the feature column register, \texttt{0xffff} is a constant for the weight, $F_{jk}$ is a pixel value, and $|i-c|$ is the distance from the average coordinate.
\par
A detail to notice in the weighted centroid calculation is the requirement of pixel brightness values. The image processing and analysis has until this point used the output of the previous step as input to the next step. The brightness data used for weighted centroid calculation comes from the output of box filter. This image is used because the pixels have the same geometry as they have in the feature table, and contain pixel brightness information.
\par
...
\par
Figure \ref*{fig:centroids} shows an example of the result of feature detection and centroiding. Detected features are rendered with a rectangular frame around the edges, with a small offset from the coordinates of the feature table, for better visibility. Both centroid variants are rendered as a cross on the corresponding feature. The non-weighted variant has a cross extending from the feature frame, while the cross of the weighted variant is significantly smaller. This helps with distinguishing the variants, and visual analysis of the weighted centroid.
...
\par
The output image can be viewed after it is transferred from the FPGA-module to the client endpoint. The analysis of feature detection is made simple by having the input and output images side by side. The zoom and movement features enhances the user experience further. The detected stars are distinguished by the frame outline and centroid, as seen in Figure \ref*{fig:centroids}. 

\chapter{Results and comparison}

The output of the pre-processing pipeline is the data generated by the system processing an image. The user-friendly validation and verification of the success of the process is done using the client endpoint. The raw data, generated by each process in the pipeline, is used to rendered an image, which leads to it containing more information than the raw data separately. The output image is thus used for result analysis.
...
\par
Implementation of requirement \hyperref[req3.2]{3.2} could be revised to increase flexibility regarding feature size. The development was focused on the computation of the weighted centroid, leaving the operational functionality as a lower priority.
\par
Requirement \hyperref[req3.3]{3.3} could be implemented more thoroughly in the FPGA design. Even though error handling is enabled by exposing parameters in the pre-processing pipeline, some error correction could also be implemented.
\par
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        ...
        \hline
    \end{tabular}
    \caption{FPGA resource utilization. The number LUT's, slices, DSP blocks and BRAM arrays are listed.}
...
\begin{figure}[h]
    ...
    \caption{Utilization \% of available FPGA resources. Total LUT utilization and LUT's used as storage mechanism, LUTRAM, is shown. Together with flip flop utilization, the internal utilization of CLB's are described. The more specialized resources, BRAM and DSP's are also reported.}
    \label{fig:fpga_util}
\end{figure}

As seen in Table \ref*{table:fpga_res}, the size of the FPGA design is reported to be 24131 LUT's, and 9242 flip flops, contributing to the 83\% slice utilization on the FPGA. The ratio between flip flops and LUT's is large due to the nature of the CLB's in Xilinx 7 series FPGA's, where each CLB contains more flip flops than LUT's.
\par
In addition to LUT's, Figure \ref*{fig:fpga_util} shows that also BRAM utilization is high, at almost 100\%. A part of the BRAM is utilized by the FIFO buffer used for image transfer, while remaining parts are used by the line buffers. The FIFO buffer uses 38Kb BRAM arrays and the line buffer uses 18Kb arrays. The line buffer uses the 18Kb arrays to enable the multiport feature of the storage mechanism.
\par
...
\par
The low level storage mechanisms have a low utilization compared to the BRAM and overall LUT utilization. This can be seen in Table \ref*{table:fpga_res}, where the count of SLICEM-type slices is relatively low compared to SLICEL. This correlates highly with the LUTRAM utilization in Figure \ref*{fig:fpga_util}.
\par
The low utilization of DSP blocks is not unexpected because the parts of the design that most likely use these blocks are not parallelized. The pre-processing pipeline performance could be increased with higher utilization of the DSP blocks, with the penalty being a higher power consumption. 
\par
The conclusion of the FPGA design based on the reports generated by Vivado is that the high degree of utilization of resources prohibits adding features requiring a large amount of LUT's or BRAM on the same FPGA. Optimizing the design to use more DSP blocks could release more LUT's to use.
