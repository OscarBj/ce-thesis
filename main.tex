...
\par
To select an image for the pre-processing pipeline, it is loaded to the application. The 270 x 480 resolution, 8-bit single channel bitmap file format is used. The image is prepared by converting it to a byte array of integers. This is the format required by the Pipe In module. The image transfer is initialized on the FPGA by configuring its state with Wire In values listed in Table \ref*{table:wireInState}, which resets the FIFO buffers and sets the SDRAM to read mode.
The write is the performed using the Block Pipe in module, by calling the \texttt{okCFrontPanel} class.
\par
