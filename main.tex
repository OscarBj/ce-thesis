...
\par
The FPGA is generally not suited for storing the bulkiest type of data. This results in inefficient usage of the available FPGA fabric, which could be used for other logic and computation. Off chip memory is instead used to allow storage of large data, freeing up FPGA fabric to other usage. Any off chip component is usually the bottleneck of a FPGA based system.

\subsection{SDRAM}
\par
The XEM7305 features a 512MiB Micron DDR3 SDRAM chip with a 32-bit wide interface. The double data rate nature of the memory works by transferring two words per clock cycle, which in this case is 32-bit. This means that for this memory part, one word is 16-bit. Another typical feature for this type of memory is operating in burst mode which 
