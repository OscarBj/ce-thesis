...
\par
The Opal Kelly register bridge connects the host interface directly to a dedicated address space on the FPGA. The register bridge is connected to 32 addresses, with 32 bits of storage each to use synchronously. The register bridge is used trough the Opal Kelly API in the client software.  
\par
Wire In and Wire Out modules are directional 32-bit buses that transfer data asynchronously. The Wire In/Out modules are by their asynchronous nature and limited capacity good for state management and triggering events. The asynchronous data is still in many cases synchronized to a clock in the design.
\par
...
The client uses the Wire In module to control  state of  is managed by Wire In/Out buses to allow a simple and reactive user experience. The Wire In uses only the first 4 bits of the 32-bit data. Table \ref{table:wireState} lists the function of each client controllable state. The wire values are set so that only one state can occur at a given moment. This maintains predictability in the system.
...
\subsubsection{Image transfer and storage}

\subsubsection{Image column data buffer}
