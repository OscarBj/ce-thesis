...
Both the USB 3 standard and the FX3 peripheral controller supports serial and parallel communication but the hard IP core connecting to the FX3 interface supports only serial communication. This simplifies the use of the IP core, requiring less FPGA fabric, but also reduces the throughput.
% USB C & 3 source needed 
\par
...
\par
An overview of the XEM7305 module and the memory types can be seen in figure \ref{fig:9} where the off-chip SDRAM memory is on the top right and the lower level memories inside the Xilinx Spartan 7 FPGA in the center.
...
The lowest level of memory found in FPGAs is distributed RAM. It is a type of on-chip memory which utilizes the same CLBs that is common with other logic of the FPGA design. This makes the use of distributed RAM a tradeoff between fast memory and CLB resources. If the FPGA design is large there might not be room for distributed RAM, leading to slightly less performant BRAM to be used instead.
% SLICEL SLICEM
The slices in Xilinx CLBs are either of type SLICEL or SLICEM. Distributed memory is available only on SLICEM type of slices. In the FPGA specification the number of each type is often mentioned.
\par
Since the distributed RAM resides in the FPGA fabric, it communicates trough the FPGA interconnect making read and write operations fast. Read and write operations require a single clock cycle to complete, making it also very simple to use from a HDL design perspective.  

\section{Clock}
As most digital hardware systems, FPGAs use clocks to synchronize operations. The clock is an off-chip component, providing the signal trough input/output pads to the FPGA. The clock signal is generated by an oscillator circuit that produces a specific frequency. This frequency may then be used as is or transformed into additional clock signals within a HDL design or IP core. For example, the DDR SDRAM utilize the clock signal on both the rising and falling edge to increase the throughput of data. The clock signal on the XEM7305 is passed from the external clock module to the SDRAM module within the HDL design. This ensures that the SDRAM and FPGA design is synchronized.
\par
The clock module on the XEM7305 provides a 200MHz frequency signal. An additional 100.8MHz clock signal is provided by the USB interface, which is required interacting with the USB peripheral controller. A third clock is used by the SDRAM, which is derived from the 200MHz clock module.
\par
In addition to using the clock signal to drive tasks, a common practice within system modeling with a HDL is to drive tasks using signals that occur more spontaneously. This could for example be a signal originating from a sensor on the FPGA module that has an interface to the FPGA design without a dependence to a clock.
